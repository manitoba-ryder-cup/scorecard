...
an FK to `matches(course_id, tee_color_id, id)`, so swapping either under a scored match
breaks it — which is why the route is a narrow sub-resource PUT rather than a full match
update.
